...
import { Plugin, Versions, InstalledPlugin } from '@/api/server/plugins-sl/Plugins';
import getVersions from '@/api/server/plugins-sl/getVersions';
...
const ModalContent = ({ plugin, framework, visible, onDismissed, ...props }: RequiredModalProps & { plugin: Plugin; framework: string }) => {
    ...
    useEffect(() => {
        if(visible) {
            ...
            getVersions({id, service: framework, pluginId: plugin.id })
...
export default function InstallButton({ plugin, service: framework }: { plugin: Plugin; service: string }) {
    const { t } = useTranslation('arix/server/addons/plugins');
